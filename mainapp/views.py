...
def home(request):
    ...
    # 🏢 Total recruiters (unique companies)
    total_recruiters = Alumni.objects.exclude(
        company_name__isnull=True
    ).exclude(
        company_name__exact=""
    ).values('company_name').distinct().count()

    # 🏆 Top recruiters (most hiring companies)
    recruiter_companies = (
        Alumni.objects.exclude(company_name__isnull=True)
    ...
    latest_year = Alumni.objects.aggregate(
        Max('graduation_year')
    )['graduation_year__max']

    # 🧑‍🎓 Recent students
    recent_alumni = Alumni.objects.select_related(
        'department'
